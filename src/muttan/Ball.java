...
//Todo 跳ね返った時の処理をちゃんとする！！！！
        ...
        x += (float) Math.sin(angle + Math.PI * 1 / 2) * speed * delta;
        y += (float) Math.cos(angle + Math.PI * 1 / 2) * speed * delta;
        ...
        if (!targets.isEmpty()) {
            ...
            //上に跳ね返るとき
            if ((leftBottom && !(leftUpper)) && (angle <= Math.PI / 2 || angle > Math.PI * 3 / 2)) {
                angle = (float) Math.PI - angle;

            } else if ((rightBottom && !(rightUpper)) && (angle <= Math.PI / 2 || angle > Math.PI * 3 / 2)) {
                angle = (float) Math.PI - angle;

            }
            //下に跳ね返るとき
            if ((leftUpper && !(leftBottom)) && (angle > Math.PI / 2 && angle <= Math.PI * 3 / 2)) {
                angle = (float) Math.PI - angle;

            } else if ((rightUpper && !(rightBottom)) && (angle > Math.PI / 2 && angle <= Math.PI * 3 / 2)) {
                angle = (float) Math.PI - angle;

            }
            //右に跳ね返るとき
            if ((leftUpper && !(rightUpper)) && (angle > Math.PI && angle <= Math.PI * 2)) {
                angle = (float) Math.PI * 2 - angle;
            } else if ((leftBottom && !(rightBottom)) && (angle > Math.PI && angle <= Math.PI * 2)) {
                angle = (float) Math.PI * 2 - angle;
            }
            //左に跳ね返るとき
            if ((rightUpper && !(leftUpper)) && (angle <= Math.PI && angle > 0)) {
                angle = (float) Math.PI * 2 - angle;
            } else if ((rightBottom && !(leftBottom)) && (angle <= Math.PI && angle > 0)) {
                angle = (float) Math.PI * 2 - angle;
            }
            if (angle < 0) {
                angle += 2 * Math.PI;
